...
import { 
  ...
  Truck, User, MapPin, CheckCircle2, Map as MapIcon, Smartphone, Building2
} from "lucide-react";
...
export default function Checkout() {
  ...
  const [paymentMode, setPaymentMode] = useState<'card' | 'momo' | 'bank'>('card');
      ...
      <div className="max-w-5xl mx-auto px-4">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2">
            ...
            {step === 4 && (
              <Card className="rounded-[2.5rem] p-8 border-none shadow-xl">
                <CardHeader className="px-0"><CardTitle className="text-3xl font-black italic">Payment.</CardTitle></CardHeader>
                <div className="space-y-6">
                  <RadioGroup value={paymentMode} onValueChange={(v: any) => { setPaymentMode(v); if(v === 'card') initializePayment(isBokoo); }} className="grid grid-cols-1 gap-4">
                    <div className={`flex items-center justify-between p-6 rounded-3xl border-2 transition-all ${paymentMode === 'card' ? 'border-black bg-gray-50' : 'border-gray-100'}`}>
                      <div className="flex items-center gap-4">
                        ...
                        <div><p className="font-black uppercase text-xs">Credit / Debit Card</p><p className="text-[10px] text-gray-400">Secure Stripe Payment</p></div>
                      </div>
                    ...
                    <div className={`flex items-center justify-between p-6 rounded-3xl border-2 transition-all ${paymentMode === 'momo' ? 'border-black bg-gray-50' : 'border-gray-100'}`}>
                      <div className="flex items-center gap-4">
                        ...
                        <div><p className="font-black uppercase text-xs">Mobile Money</p><p className="text-[10px] text-gray-400">MTN, Telecel, AT</p></div>
                      </div>
                  ...
                  {paymentMode === 'card' && clientSecret && (
                    <Elements stripe={stripePromise} options={{ clientSecret }}>
                      <CheckoutForm isBokoo={isBokoo} originalTotal={cartTotal} checkoutDetails={details} />
                    </Elements>
                  )}
                  ...
                  {paymentMode !== 'card' && (
                    <div className="space-y-4">
                      <div className="bg-amber-50 p-6 rounded-3xl border border-amber-100">
                        <p className="text-xs font-bold text-amber-900 mb-2">Instructions:</p>
                        <p className="text-[11px] text-amber-800 leading-relaxed">
                          {paymentMode === 'momo' 
                            ? "Complete your order and follow the prompt on your phone to authorize payment."
                            : "Transfer the grand total to the university hub bank account and upload your receipt."}
                        </p>
                      </div>
                      <Button onClick={handleManualOrder} className="w-full h-16 rounded-2xl bg-black text-white font-black text-lg">Confirm Order</Button>
                    </div>
                  )}
                </div>
              </Card>
