...
import { 
  ...
  insertWeeklyDealSchema, insertCampusActivitySchema, insertSellerReviewSchema,
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.put("/api/orders/:id/buyer-confirmation", authenticateToken, async (req: AuthRequest, res) => {
    try {
      ...
      if (confirmation === 'received') {
        const feedbackUrl = `${process.env.APP_URL || 'https://uniexchangehub.com'}/dashboard?tab=purchases&reviewOrderId=${id}`;
        
        await sendLocalEmail(order.buyer.email, 'Thank You!', `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e5e7eb; rounded: 12px;">
            ...
            <p>Your order for <strong>${order.product.title}</strong> has been successfully delivered and confirmed.</p>
            <p>We'd love to hear your feedback about the product and the seller. Your review helps other students shop with confidence.</p>
            <div style="margin: 30px 0; text-align: center;">
              <a href="${feedbackUrl}" style="background-color: #000; color: #fff; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-weight: bold; font-size: 14px;">Submit a Review</a>
            </div>
            <p style="color: #6b7280; font-size: 12px;">Thank you for being a part of The Hub community.</p>
          </div>
        `);
  ...
  app.post("/api/reviews", authenticateToken, async (req: AuthRequest, res) => {
    try {
      const reviewData = insertSellerReviewSchema.parse(req.body);
      
      // Ensure reviewerId matches authenticated user
      if (reviewData.reviewerId !== req.userId) {
        return res.status(403).json({ message: "Unauthorized review submission" });
      }

      const review = await storage.createSellerReview(reviewData);
      
      // Notify Seller via Inbox
      const reviewer = await storage.getUserById(req.userId!);
      const product = reviewData.productId ? await storage.getProductById(reviewData.productId) : null;
      
      await storage.createMessage({
        fromId: 1, // Hub Official
        toId: reviewData.sellerId,
        content: `🎉 New Review! ${reviewer?.firstName} just left a ${reviewData.rating}-star review for ${product?.title || 'your store'}: "${reviewData.comment}"`,
      });

      // Notify Seller & Admin via Email
      const { sendAdminAlert } = await import('./notifications');
      const seller = await storage.getUserById(reviewData.sellerId);
      
      if (seller) {
        await sendLocalEmail(seller.email, 'New Customer Review!', `
          <div style="font-family: sans-serif; padding: 20px; border: 1px solid #eee; border-radius: 10px;">
            <h2 style="color: #6366f1;">New ${reviewData.rating}-Star Review</h2>
            <p><strong>Reviewer:</strong> ${reviewer?.firstName} ${reviewer?.lastName}</p>
            <p><strong>Comment:</strong> "${reviewData.comment}"</p>
            <p>This review is now live on your store profile.</p>
          </div>
        `);
      }

      await sendAdminAlert('New Platform Review', `${reviewer?.firstName} reviewed Seller ID: ${reviewData.sellerId} with ${reviewData.rating} stars.`);
  ...
  app.get("/api/reviews/seller/:sellerId", async (req, res) => {
    try {
      const reviews = await storage.getReviewsBySellerId(parseInt(req.params.sellerId));
      res.json(reviews);
    } catch (error) {
  ...
  app.put("/api/admin/users/:id/verify", authenticateToken, requireAdmin, async (req: AuthRequest, res) => {
    try {
      ...
      if (status === 'rejected') {
        ...
        await sendLocalEmail(user.email, 'Seller Application Rejected - The University Hub', rejectionContent);
      ...
      if (status === 'verified') {
        ...
        const approvedContent = `
          ...
          <p style="color: #6b7280; font-size: 14px;">1. Visit our homepage.<br>
        ...
        await sendLocalEmail(user.email, 'Seller Application Approved! - The University Hub', approvedContent);
        ...
        await sendLocalEmail(user.email, 'Action Required: Seller Application Correction - The University Hub', correctionContent);
      }
